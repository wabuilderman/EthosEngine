...
void main() {
	...
	vec4 weights = inBoneIndices - vec4(index0, index1, index2, index3);


	//modelPos = (
	//	bones.matrices[index0] * weights.x +
	//	bones.matrices[index1] * weights.y +
	//	bones.matrices[index2] * weights.z +
	//	bones.matrices[index3] * weights.w
	//) * modelPos;
	
	if(inBoneIndices.x >= 0) {
		mat4 deform = bones.matrices[index0] * weights.x;
		if(inBoneIndices.y >= 0) {
			deform += bones.matrices[index1] * weights.y;
			if(inBoneIndices.z >= 0) {
				deform += bones.matrices[index2] * weights.z;
				if(inBoneIndices.w >= 0) {
					deform += bones.matrices[index3] * weights.w;
				}
			}
		}

		modelPos = deform * modelPos;
	}
